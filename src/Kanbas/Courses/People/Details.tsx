...
export default function PeopleDetails() {
  ...
  const saveRole = async () => {
    ...
    await client.updateRole(user, updatedRole);
    setRole(updatedRole);
  ...
  const saveEmail = async () => {
    ...
    await client.updateEmail(user, updatedEmail);
      ...
      {!editingRole && (
        ...
        {!editingRole && (
          <span className="wd-roles"
            ...
            {user.role}</span>)}
            {user && editingRole && (
                <select value={role} onChange={(e) => setRole(e.target.value)} onKeyDown={(e) => {
                    if (e.key === "Enter") { saveRole(); }}}
                    className="form-select w-75 wd-select-role" >
                    <option value="">Please select a new role</option>
